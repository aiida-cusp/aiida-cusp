...
Similarly to the actual VASP code input, defined by the ``inputs.code`` option, additional settings for the Custodian code may be passed through the ``inputs.custodian.settings`` option to customize Custodian's behavior.
...
The available set of error handlers that can be used with this plugin is documented in the :ref:`handler section<user-guide-custodian-handlers>`. Similarly, Custodian jobs can be created and added to the calculation via the ``inputs.custodian.jobs`` input. Further information on how to setup such jobs can be found in the :ref:`jobs section<user-guide-custodian-jobs>`.

.. warning::

   Note that all custodian inputs, i.e. ``custodian.code``, ``custodian.settings``, ``custodian.handlers`` and ``custodian.jobs`` are entirely optional and do not need to be specified.
   ...
   The same applies, if no Custodian jobs are passed to the calculation, which is perfectly OK and will result in the execution of an ordinary VASP calculation.
